...
class Algorithms:
    ...
    def createA(self, class_representative=False):
        ...
        folders = sorted(
            [
                name
                for name in os.listdir(self.PATH_DS)
                if os.path.isdir(os.path.join(self.PATH_DS, name))
            ]
        )

        for i, folder in enumerate(folders, 1):
            pathFolder = self.PATH_DS + r"/" + folder + "/"
    ...
    def NNAlgorithm(self, A, testPictureVect, norm) -> int:
        ...
        for i in range(len(z)):
            if norm == 1:
                ...
                z[i] = 1 - np.dot(testPictureVect, A[:, i]) / (
                    np.linalg.norm(testPictureVect) * np.linalg.norm(A[:, i])
                )
    ...
    def RRAQT(self, A, norm, k):
        ...
        return counter / (
            self.NR_PERS * (self.NR_PICTURES_PERS - self.NR_PICTURES_TRAIN)
        ), totalTime / (self.NR_PERS * (self.NR_PICTURES_PERS - self.NR_PICTURES_TRAIN))
    ...
    def EF(self, A, k):
        ...
        d, V = np.linalg.eig(
            L
        )  # V - col as eigenvectors for L, d - vector of eigenvalues
        idx = np.argsort(d)

        idx_k = idx[
            -1 : -k - 1 : -1
        ]  # keep the last k indices, corresponding to the k largest eigenvalues
        ...
        proiectie = np.dot(
            np.transpose(A_copy), V
        )  # projection of the data on the HQPB
    ...
    def RRAQT_EF(self, A, norm, k):
        ...
        for idx, folder in enumerate(folders, 1):
            ...
            for j in range(self.NR_PICTURES_TRAIN + 1, self.NR_PICTURES_PERS + 1):
                ...
                if i0 // self.NR_PICTURES_TRAIN + 1 == idx:
                    counter += 1
            totalTime += t1 - t0
